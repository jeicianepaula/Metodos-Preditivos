...
#---------------------------------------------------------ARIMA/SARIMA----------------------------------------------------------------------------------------------
ARIMA = function (x,Data_IN,Data_OUT,Dados_reais_OUT, h){ #h = horizonte de previsão
  
    #Faz a modelagem e previsão  
    ARIMA = x %>% auto.arima(trace=T, stepwise = F, approximation = F) %>% forecast(h=h,level = c(85,95))
    print(ARIMA[["model"]])
...
ARIMA(serie,Data[1:156],Data[157:180],Dados[157:180,1], 24)
